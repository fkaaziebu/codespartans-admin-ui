"use client";
import { ArrowLeft, Eye, Lock, Plus } from "lucide-react";
...
import { useApproveCourseVersion } from "@/common/hooks/mutations";
import { useGetCourseVersion } from "@/common/hooks/queries";
import { CreateReviewModal, ViewQuestionsModal } from "@/components/modals";
...
export default function AdminVersionDetailPage() {
  ...
  const { getCourseVersion } = useGetCourseVersion();
  const { approveCourseVersion } = useApproveCourseVersion();
  ...
  const getVersionDetails = async () => {
    try {
      const response = await getCourseVersion({
        variables: {
          versionId,
        },
        fetchPolicy: "no-cache",
      ...
      setVersionDetails({
        id: response.data?.getCourseVersion.id,
        course_name: response.data?.getCourseVersion.course?.title,
        version_number: response.data?.getCourseVersion.version_number,
        status: response.data?.getCourseVersion.status,
        date_assigned:
          response.data?.getCourseVersion.review_request?.inserted_at,
        number_of_questions: response.data?.getCourseVersion.total_questions,
        instructor_name:
          response.data?.getCourseVersion.course?.instructor?.name,
      });

      const versionReviews = response.data?.getCourseVersion.reviews.map(
        (review) => ({
          id: review.id,
          title: review.title,
  ...
  const handleReviewClick = (reviewId: string) => {
    router.push(`/versions/${versionId}/reviews/${reviewId}`);
  };

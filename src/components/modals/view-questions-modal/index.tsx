...
import { useListQuestionsForVersion } from "@/common/hooks/queries";
...
export default function ViewQuestionsModal({
  ...
  const { listQuestionsForVersion } = useListQuestionsForVersion();

  const fetchQuestions = async () => {
    setLoading(true);
    try {
      // Simulate API call - replace with actual API
      const response = await listQuestionsForVersion({
        variables: {
          versionId,
          pagination: {
            first: 1000,
      ...
      const allQuestions: Question[] | undefined =
        response.data?.listQuestionsForVersion.edges.map((edge) => ({
          id: edge.node.id,
